/* solid reactivity (solid-js@1.6.14) */
...
let Effects = null
let ExecCount = 0
const [transPending, setTransPending] =
...
function useContext$2(context) {
	let ctx
	return (ctx = lookup(Owner, context.id)) !== undefined
		? ctx
...
function markDownstream(node) {
	...
	for (let i = 0; i < node.observers.length; i += 1) {
		const o = node.observers[i]
		if (
			(!runningTransition && !o.state) ||
			(runningTransition && !o.tState)
		) {
...
function SuspenseList(props) {
	let [wrapper, setWrapper] = createSignal(() => ({
		...
		show
	const listContext = useContext$2(SuspenseListContext)
	const [registry, setRegistry] = createSignal([])
	if (listContext) {
		show = listContext.register(
			createMemo(() => wrapper()().inFallback),
		)
	}
	const resolved = createMemo(
		prev => {
			const reveal = props.revealOrder,
				tail = props.tail,
				{ showContent = true, showFallback = true } = show
					? show()
					: {},
				reg = registry(),
...
export {
	// core used on DOM Expressions
	createRoot as root,
	createEffect$1 as effect,
	untrack,
...
export {
	// reactivity
	NotImplemented as computed,
	NotImplemented as memo,

	// context
	NotImplemented as createContext,
	NotImplemented as useContext,

	// helpers
	NotImplemented as createSelector,
	NotImplemented as lazy,

	// props stuff
	NotImplemented as splitProps,
	NotImplemented as mergeProps,

	// flow
	NotImplemented as map,
}
